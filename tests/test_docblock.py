...
from databasetools.utils.docBlock.md2docBlock import Md2DocBlock, InvalidTokenError, NotRelativeURIWarning
from databasetools.utils.docBlock.docBlock2md import DocBlock2Md, InvalidBlockInput
...
class TestDocBlock2Md(unittest.TestCase):
    def setUp(self):
        ...
        self.tomd = DocBlock2Md()
        
    def test_text(self):
        test_text = "Hello there"
        block_list, _ = Md2DocBlock.parse_md2docblock(test_text)
        assert len(block_list) == 2
        text_block = block_list[1]
        
        result = self.tomd._text(text_block)
        assert result == test_text
        
        with self.assertRaises(InvalidBlockInput):
            self.tomd._text(block_list[0])
        
    def test_heading(self):
        test_heading = "# Heading!"
        block_list, _ = Md2DocBlock.parse_md2docblock(test_heading)
        assert len(block_list) == 1
        heading_block = block_list[0]
        result = self.tomd._heading(heading_block)
        assert result == test_heading
    ...
    def test_block_code(self):
        test_block_code = """```python\nfrom peanuts import Peanuts\npeanuts = Peanuts()\npeanuts.roast()\npeanuts.make_peanutbutter()\n```"""
        block_list, _ = Md2DocBlock.parse_md2docblock(test_block_code)
        assert len(block_list) == 1
        code_block = block_list[0]
        result = self.tomd._block_code(code_block)
        assert result == test_block_code
        
    def test_codespan(self):
        test_codespan = "`hello there!!!`"
        codespan_block = DocBlockElement(
            type=DocBlockElementType.CODESPAN,
            block_content="hello there!!!"
        )
        result = self.tomd._codespan(codespan_block)
        assert result == test_codespan
        
    def test_image(self):
        test_image = "![image caption here](http://A/url/here/)"
        block_list, _ = Md2DocBlock.parse_md2docblock(test_image)
        assert len(block_list) == 2
        image_block = block_list[1]
        result = self.tomd._image(image_block)
        assert result == test_image
        
    def test_link(self):
        test_link = "[LINK with some stuffs in it!](http://google.com)"
        block_list, _ = Md2DocBlock.parse_md2docblock(test_link)
        assert len(block_list) == 3
        
        link_block = block_list[1]
